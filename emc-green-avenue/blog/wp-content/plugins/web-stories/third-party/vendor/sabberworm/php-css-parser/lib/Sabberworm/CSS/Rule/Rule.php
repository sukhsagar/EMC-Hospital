...
namespace Google\Web_Stories_Dependencies\Sabberworm\CSS\Rule;
 ...
 */
class Rule implements \Google\Web_Stories_Dependencies\Sabberworm\CSS\Renderable, \Google\Web_Stories_Dependencies\Sabberworm\CSS\Comment\Commentable
{
    private $sRule;
    private $mValue;
    ...
    public static function parse(\Google\Web_Stories_Dependencies\Sabberworm\CSS\Parsing\ParserState $oParserState)
    {
        ...
        $oValue = \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Value::parseValue($oParserState, self::listDelimiterForRule($oRule->getRule()));
    ...
    public function setValues($aSpaceSeparatedValues)
    {
        ...
        if (\count($aSpaceSeparatedValues) > 1) {
            $oSpaceSeparatedList = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList(' ', $this->iLineNo);
        }
        foreach ($aSpaceSeparatedValues as $aCommaSeparatedValues) {
            $oCommaSeparatedList = null;
            if (\count($aCommaSeparatedValues) > 1) {
                $oCommaSeparatedList = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList(',', $this->iLineNo);
            }
    ...
    {
        if (!$this->mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList) {
    ...
    public function addValue($mValue, $sType = ' ')
    {
        ...
        if (!$this->mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList || $this->mValue->getListSeparator() !== $sType) {
            $mCurrentValue = $this->mValue;
            $this->mValue = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList($sType, $this->iLineNo);
            if ($mCurrentValue) {
    ...
    public function render(\Google\Web_Stories_Dependencies\Sabberworm\CSS\OutputFormat $oOutputFormat)
    {
        ...
        if ($this->mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Value) {
