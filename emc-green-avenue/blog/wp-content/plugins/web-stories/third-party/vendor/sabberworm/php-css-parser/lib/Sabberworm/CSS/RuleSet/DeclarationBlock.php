 ...
 */
class DeclarationBlock extends \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\RuleSet
{
    private $aSelectors;
    ...
    public static function parse(\Google\Web_Stories_Dependencies\Sabberworm\CSS\Parsing\ParserState $oParserState, $oList = NULL)
    {
        $aComments = array();
        $oResult = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\DeclarationBlock($oParserState->currentLine());
        try {
            ...
            do {
                ...
                if (\in_array($oParserState->peek(), array('\'', '"')) && \substr(\end($aSelectorParts), -1) != "\\") {
                    if ($sStringWrapperChar === \false) {
                        ...
                        if ($sStringWrapperChar == $oParserState->peek()) {
                            $sStringWrapperChar = \false;
                        }
                    }
                }
        ...
        \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\RuleSet::parseRuleSet($oParserState, $oResult);
        return $oResult;
    }
    public function setSelectors($mSelector, $oList = NULL)
    {
        if (\is_array($mSelector)) {
            ...
            if (empty($aPlaceholders)) {
                ...
                $this->aSelectors = \array_map(function ($sSelector) use($aSearches, $aReplaces) {
                    return \str_replace($aSearches, $aReplaces, $sSelector);
                }, \explode(',', $sSelectors));
            }
        }
        foreach ($this->aSelectors as $iKey => $mSelector) {
            if (!$mSelector instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\Selector) {
                if ($oList === NULL || !$oList instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\KeyFrame) {
                    if (!\Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\Selector::isValid($mSelector)) {
                        throw new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Parsing\UnexpectedTokenException("Selector did not match '" . \Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\Selector::SELECTOR_VALIDATION_RX . "'.", $mSelector, "custom");
                    }
                    $this->aSelectors[$iKey] = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\Selector($mSelector);
                } else {
                    if (!\Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\KeyframeSelector::isValid($mSelector)) {
                        throw new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Parsing\UnexpectedTokenException("Selector did not match '" . \Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\KeyframeSelector::SELECTOR_VALIDATION_RX . "'.", $mSelector, "custom");
                    }
                    $this->aSelectors[$iKey] = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\KeyframeSelector($mSelector);
                }
            }
        }
    }
    ...
    public function expandBorderShorthand()
    {
        ...
        foreach ($aBorderRules as $sBorderRule) {
            ...
            $aValues = array();
            if (!$mRuleValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList) {
                $aValues[] = $mRuleValue;
            } else {
                $aValues = $mRuleValue->getListComponents();
            }
            foreach ($aValues as $mValue) {
                if ($mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Value) {
                    $mNewValue = clone $mValue;
                } else {
                    $mNewValue = $mValue;
                }
                if ($mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size) {
                    $sNewRuleName = $sBorderRule . "-width";
                } else {
                    if ($mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Color) {
                        $sNewRuleName = $sBorderRule . "-color";
                    } else {
                        if (\in_array($mValue, $aBorderSizes)) {
                            $sNewRuleName = $sBorderRule . "-width";
                        } else {
                            /* if(in_array($mValue, $aBorderStyles)) */
    ...
    public function expandFontShorthand()
    {
        ...
        foreach ($aValues as $mValue) {
            ...
            if (\in_array($mValue, array('normal', 'inherit'))) {
                ...
                if (\in_array($mValue, array('italic', 'oblique'))) {
                    $aFontProperties['font-style'] = $mValue;
                } else {
                    if ($mValue == 'small-caps') {
                        $aFontProperties['font-variant'] = $mValue;
                    } else {
                        if (\in_array($mValue, array('bold', 'bolder', 'lighter')) || $mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size && \in_array($mValue->getSize(), \range(100, 900, 100))) {
                            $aFontProperties['font-weight'] = $mValue;
                        } else {
                            if ($mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList && $mValue->getListSeparator() == '/') {
                                list($oSize, $oHeight) = $mValue->getListComponents();
                                $aFontProperties['font-size'] = $oSize;
                                $aFontProperties['line-height'] = $oHeight;
                            } else {
                                if ($mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size && $mValue->getUnit() !== null) {
    ...
    public function expandBackgroundShorthand()
    {
        ...
        $aBgProperties = array('background-color' => array('transparent'), 'background-image' => array('none'), 'background-repeat' => array('repeat'), 'background-attachment' => array('scroll'), 'background-position' => array(new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size(0, '%', null, \false, $this->iLineNo), new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size(0, '%', null, \false, $this->iLineNo)));
        $mRuleValue = $oRule->getValue();
        $aValues = array();
        if (!$mRuleValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList) {
            $aValues[] = $mRuleValue;
        ...
        foreach ($aValues as $mValue) {
            ...
            if ($mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\URL) {
                $aBgProperties['background-image'] = $mValue;
            } else {
                if ($mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Color) {
                    $aBgProperties['background-color'] = $mValue;
                } else {
                    if (\in_array($mValue, array('scroll', 'fixed'))) {
                        $aBgProperties['background-attachment'] = $mValue;
                    } else {
                        if (\in_array($mValue, array('repeat', 'no-repeat', 'repeat-x', 'repeat-y'))) {
                            $aBgProperties['background-repeat'] = $mValue;
                        } else {
                            if (\in_array($mValue, array('left', 'center', 'right', 'top', 'bottom')) || $mValue instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Size) {
                                if ($iNumBgPos == 0) {
    ...
    public function createFontShorthand()
    {
        ...
        if (isset($aRules['line-height'])) {
            ...
            if ($aLHValues[0] !== 'normal') {
                $val = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList('/', $this->iLineNo);
        ...
        $oFFValue = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList(',', $this->iLineNo);
    ...
    public function render(\Google\Web_Stories_Dependencies\Sabberworm\CSS\OutputFormat $oOutputFormat)
    {
        if (\count($this->aSelectors) === 0) {
            ...
            throw new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Parsing\OutputException("Attempt to print declaration block with missing selector", $this->iLineNo);
        }
