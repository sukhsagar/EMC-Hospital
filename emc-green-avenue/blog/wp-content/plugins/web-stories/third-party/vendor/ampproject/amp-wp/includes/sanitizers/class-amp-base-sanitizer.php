...
abstract class AMP_Base_Sanitizer
{
    ...
    {
        \Google\Web_Stories_Dependencies\_deprecated_function('Use $this->dom->body instead', '1.5.0');
    ...
    public function sanitize_dimension($value, $dimension)
    {
        ...
        if (\Google\Web_Stories_Dependencies\AMP_String_Utils::endswith($value, 'px')) {
            return \Google\Web_Stories_Dependencies\absint($value);
        }
        if (\Google\Web_Stories_Dependencies\AMP_String_Utils::endswith($value, '%') && 'width' === $dimension && isset($this->args['content_max_width'])) {
            $percentage = \Google\Web_Stories_Dependencies\absint($value) / 100;
    ...
    public function maybe_enforce_https_src($src, $force_https = \false)
    {
        ...
        if ('https' !== $protocol) {
            ...
            if (isset($this->args['require_https_src']) && \true === $this->args['require_https_src']) {
                ...
                $src = \Google\Web_Stories_Dependencies\set_url_scheme($src, 'https');
            }
        }
        return $src;
    }
    ...
    {
        \Google\Web_Stories_Dependencies\_deprecated_function('AMP_Base_Sanitizer::is_document_in_dev_mode', '1.5', 'AmpProject\\DevMode::isActiveForDocument');
        return \Google\Web_Stories_Dependencies\AmpProject\DevMode::isActiveForDocument($this->dom);
    }
    /**
    ...
    {
        \Google\Web_Stories_Dependencies\_deprecated_function('AMP_Base_Sanitizer::has_dev_mode_exemption', '1.5', 'AmpProject\\DevMode::hasExemptionForNode');
        return \Google\Web_Stories_Dependencies\AmpProject\DevMode::hasExemptionForNode($node);
    }
    /**
    ...
    {
        \Google\Web_Stories_Dependencies\_deprecated_function('AMP_Base_Sanitizer::is_exempt_from_validation', '1.5', 'AmpProject\\DevMode::isExemptFromValidation');
        return \Google\Web_Stories_Dependencies\AmpProject\DevMode::isExemptFromValidation($node);
    }
    /**
    ...
    {
        if (\Google\Web_Stories_Dependencies\AmpProject\DevMode::isExemptFromValidation($node)) {
            return \false;
        }
        // Prevent double-reporting nodes that are rejected for sanitization.
        if (isset($this->nodes_to_keep[$node->nodeName]) && \in_array($node, $this->nodes_to_keep[$node->nodeName], \true)) {
            return \false;
        }
    ...
    {
        if (\Google\Web_Stories_Dependencies\AmpProject\DevMode::isExemptFromValidation($element)) {
            return \false;
        }
    ...
    public function prepare_validation_error(array $error = [], array $data = [])
    {
        ...
        if ($node instanceof \DOMElement) {
            ...
            if ('script' === $node->nodeName && !$node->hasAttribute('src') || 'style' === $node->nodeName && !$node->hasAttribute('amp-custom') && !$node->hasAttribute('amp-keyframes')) {
    ...
    public function get_data_amp_attributes($node)
    {
        ...
        if ($parent_node instanceof \Google\Web_Stories_Dependencies\DOMELement && 'figure' === $parent_node->tagName) {
