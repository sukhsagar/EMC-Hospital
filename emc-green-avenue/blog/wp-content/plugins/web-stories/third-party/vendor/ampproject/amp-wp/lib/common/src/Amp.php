...
final class Amp
{
    ...
    {
        if (!$node instanceof \DOMElement || !self::isAsyncScript($node) || self::isExtension($node)) {
            return \false;
        }
        ...
        if (\substr($src, -6) !== '/v0.js' && \substr($src, -14) !== '/amp4ads-v0.js') {
            return \false;
        }
        return \true;
    }
    ...
    {
        if (!$node instanceof \DOMElement || $node->tagName !== \Google\Web_Stories_Dependencies\AmpProject\Tag::SCRIPT) {
            return '';
        }
    ...
    {
        return $node instanceof \DOMElement && \strpos($node->tagName, \Google\Web_Stories_Dependencies\AmpProject\Extension::PREFIX) === 0;
    }
    /**
