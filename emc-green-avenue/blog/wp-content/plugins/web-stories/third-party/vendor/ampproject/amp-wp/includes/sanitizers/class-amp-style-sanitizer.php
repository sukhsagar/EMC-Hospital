...
class AMP_Style_Sanitizer extends \Google\Web_Stories_Dependencies\AMP_Base_Sanitizer
{
     ...
     * @var array
     */
    protected $DEFAULT_ARGS = ['dynamic_element_selectors' => ['amp-list', 'amp-live-list', '[submit-error]', '[submit-success]', 'amp-script'], 'should_locate_sources' => \false, 'parsed_cache_variant' => null, 'focus_within_classes' => ['focus'], 'low_priority_plugins' => ['query-monitor'], 'allow_transient_caching' => \true];
    /**
    ...
    public function __construct($dom, array $args = [])
    {
        ...
        $this->remote_request = new \Google\Web_Stories_Dependencies\AmpProject\AmpWP\RemoteRequest\CachedRemoteGetRequest(new \Google\Web_Stories_Dependencies\AmpProject\AmpWP\RemoteRequest\WpHttpRemoteGetRequest());
    }
    /**
    ...
    private function get_used_class_names()
    {
        ...
        foreach ($this->dom->xpath->query('//*/@' . \Google\Web_Stories_Dependencies\AmpProject\Dom\Document::AMP_BIND_DATA_ATTR_PREFIX . 'class') as $bound_class_attribute) {
    ...
    public function sanitize()
    {
         ...
         */
        $dev_mode_predicate = '';
        if (\Google\Web_Stories_Dependencies\AmpProject\DevMode::isActiveForDocument($this->dom)) {
    ...
    private function process_style_element(\DOMElement $element)
    {
        ...
        $this->pending_stylesheets[] = ['group' => $is_keyframes ? self::STYLE_AMP_KEYFRAMES_GROUP_INDEX : self::STYLE_AMP_CUSTOM_GROUP_INDEX, 'original_size' => (int) \strlen($stylesheet), 'final_size' => null, 'element' => $element, 'origin' => 'style_element', 'sources' => $this->current_sources, 'priority' => $this->get_stylesheet_priority($element), 'tokens' => $parsed['tokens'], 'hash' => $parsed['hash'], 'parse_time' => $parsed['parse_time'], 'shake_time' => null, 'cached' => $parsed['cached'], 'imported_font_urls' => $parsed['imported_font_urls']];
    ...
    private function fetch_external_stylesheet($url)
    {
        ...
        try {
            ...
            if ($exception instanceof \Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToGetFromRemoteUrl && $exception->hasStatusCode()) {
    ...
    private function get_parsed_stylesheet($stylesheet, $options = [])
    {
        ...
        $cache_key = null;
        $cached = \true;
        $cache_group = 'amp-parsed-stylesheet-v34';
        ...
        $cache_impacting_options = \array_merge(\wp_array_slice_assoc($options, ['property_allowlist', 'property_denylist', 'stylesheet_url', 'allowed_at_rules']), \wp_array_slice_assoc($this->args, ['should_locate_sources', 'parsed_cache_variant', 'dynamic_element_selectors']), ['language' => \get_bloginfo('language')]);
        ...
        if (!$parsed || !isset($parsed['tokens']) || !\is_array($parsed['tokens'])) {
            ...
            if ($use_transients) {
                ...
                \set_transient($cache_group . '-' . $cache_key, $parsed, \Google\Web_Stories_Dependencies\MONTH_IN_SECONDS);
            } else {
    ...
    private function should_use_transient_caching()
    {
        ...
        if (\Google\Web_Stories_Dependencies\AMP_Options_Manager::get_option(\Google\Web_Stories_Dependencies\AmpProject\AmpWP\Option::DISABLE_CSS_TRANSIENT_CACHING, \false)) {
            return \false;
        }
        return \true;
    }
     ...
     * }
     */
    private function splice_imported_stylesheet(\Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\Import $item, \Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\CSSList $css_list, $options)
    {
        $validation_results = [];
        $imported_font_urls = [];
        $viewport_rules = [];
     ...
     */
    private function replace_inside_css_list(\Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\CSSList $css_list, $old_item, $new_items = [])
    {
        ...
        try {
            ...
            if (!empty($options['stylesheet_url'])) {
                $this->real_path_urls(\array_filter($css_document->getAllValues(), static function ($value) {
                    return $value instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\URL;
    ...
    private function parse_stylesheet($stylesheet_string, $options = [])
    {
        ...
        return \array_merge(\compact('tokens', 'validation_results'), ['imported_font_urls' => $parsed_stylesheet['imported_font_urls'], 'hash' => \md5(\wp_json_encode($tokens)), 'parse_time' => \microtime(\true) - $start_time, 'viewport_rules' => $parsed_stylesheet['viewport_rules']]);
    }
    /**
     ...
     * }
     */
    private function process_css_list(\Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\CSSList $css_list, $options)
    {
        $validation_results = [];
        $viewport_rules = [];
        $imported_font_urls = [];
        foreach ($css_list->getContents() as $css_item) {
            $sanitized = \false;
            if ($css_item instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\DeclarationBlock && empty($options['validate_keyframes'])) {
                $validation_results = \array_merge($validation_results, $this->process_css_declaration_block($css_item, $css_list, $options));
            } elseif ($css_item instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\AtRuleBlockList) {
                if ('-moz-document' === $css_item->atRuleName() && 'url-prefix()' === $css_item->atRuleArgs() && \in_array('supports', $options['allowed_at_rules'], \true)) {
                    ...
                    $new_css_item = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\AtRuleBlockList('supports', '(-moz-appearance:meterbar)');
                    $new_css_item->setContents($css_item->getContents());
                    $this->replace_inside_css_list($css_list, $css_item, [$new_css_item]);
                    $css_item = $new_css_item;
                    // To process_css_list below.
                    $sanitized = \false;
                ...
                if (!$sanitized) {
                    ...
                    $validation_results = \array_merge($validation_results, $at_rule_processed_list['validation_results']);
                }
            } elseif ($css_item instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\Import) {
            ...
            } elseif ($css_item instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\AtRuleSet) {
                if (\preg_match('/^(-.+-)?viewport$/', $css_item->atRuleName())) {
                    $output_format = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\OutputFormat();
                    foreach ($css_item->getRules() as $rule) {
                        $rule_value = $rule->getValue();
                        if ($rule_value instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\Value) {
            ...
            } elseif ($css_item instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\KeyFrame) {
                ...
                if (!$sanitized) {
                    $validation_results = \array_merge($validation_results, $this->process_css_keyframes($css_item, $options));
                }
            } elseif ($css_item instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\AtRule) {
     ...
     */
    private function process_css_declaration_block(\Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\RuleSet $ruleset, \Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\CSSList $css_list, $options)
    {
        $results = [];
        if ($ruleset instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\DeclarationBlock) {
            $results = \array_merge($results, $this->ampify_ruleset_selectors($ruleset));
        ...
        if ($ruleset instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\AtRuleSet && 'font-face' === $ruleset->atRuleName()) {
            $this->process_font_face_at_rule($ruleset, $options);
        }
        $results = \array_merge($results, $this->transform_important_qualifiers($ruleset, $css_list, $options));
     ...
     * }
     */
    private function process_font_face_at_rule(\Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\AtRuleSet $ruleset, $options)
    {
        ...
        foreach ($src_properties as $src_property) {
            ...
            if (!$value instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList) {
                continue;
            }
            /*
            ...
            foreach ($value->getListComponents() as $component) {
                if ($component instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\RuleValueList) {
            ...
            foreach ($sources as $i => $source) {
                if ($source[0] instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\Value\URL) {
            ...
            foreach ($source_data_url_objects as $i => $data_url) {
                ...
                if (\in_array($font_filename, $bundled_fonts, \true)) {
                    $data_url->getURL()->setString(\plugin_dir_url(\Google\Web_Stories_Dependencies\AMP__FILE__) . "assets/fonts/{$font_filename}");
                    $converted_count++;
                }
            }
        ...
        if ($converted_count && 0 === \count($ruleset->getRules('font-display'))) {
            $font_display_rule = new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Rule\Rule('font-display');
     ...
     * @return array Validation results.
     */
    private function process_css_keyframes(\Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\KeyFrame $css_list, $options)
    {
        $results = [];
        if (!empty($options['property_allowlist'])) {
            foreach ($css_list->getContents() as $rules) {
                if (!$rules instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\DeclarationBlock) {
                ...
                $results = \array_merge($results, $this->transform_important_qualifiers($rules, $css_list, $options));
     ...
     * @return array Validation results.
     */
    private function transform_important_qualifiers(\Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\RuleSet $ruleset, \Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\CSSList $css_list, $options)
    {
        $results = [];
        // An !important only makes sense for rulesets that have selectors.
        $allow_transformation = $ruleset instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\DeclarationBlock && !$css_list instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\CSSList\KeyFrame;
        $properties = $ruleset->getRules();
        $importants = [];
        foreach ($properties as $property) {
        ...
        if (!$ruleset instanceof \Google\Web_Stories_Dependencies\Sabberworm\CSS\RuleSet\DeclarationBlock || !$allow_transformation || empty($importants)) {
            return $results;
        }
        /**
        ...
        $important_ruleset->setSelectors(\array_map(
             ...
             */
            static function (\Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\Selector $old_selector) {
                ...
                return new \Google\Web_Stories_Dependencies\Sabberworm\CSS\Property\Selector($new_selector);
            },
    ...
    private function collect_inline_styles(\DOMElement $element)
    {
        ...
        if ($parsed['tokens']) {
            $this->pending_stylesheets[] = ['group' => self::STYLE_AMP_CUSTOM_GROUP_INDEX, 'original_size' => \strlen($rule), 'final_size' => null, 'element' => $element, 'origin' => 'style_attribute', 'sources' => $this->current_sources, 'priority' => $this->get_stylesheet_priority($attr_node), 'tokens' => $parsed['tokens'], 'hash' => $parsed['hash'], 'parse_time' => $parsed['parse_time'], 'shake_time' => null, 'cached' => $parsed['cached']];
    ...
    {
        $stylesheet_groups = [self::STYLE_AMP_CUSTOM_GROUP_INDEX => ['source_map_comment' => "\n\n/*# sourceURL=amp-custom.css */", 'cdata_spec' => $this->style_custom_cdata_spec, 'included_count' => 0, 'import_front_matter' => ''], self::STYLE_AMP_KEYFRAMES_GROUP_INDEX => ['source_map_comment' => "\n\n/*# sourceURL=amp-keyframes.css */", 'cdata_spec' => $this->style_keyframes_cdata_spec, 'included_count' => 0, 'import_front_matter' => '']];
        $imported_font_urls = [];
    ...
    public function add_css_budget_to_admin_bar()
    {
        ...
        if ($css_usage_percentage > 100) {
            ...
            $icon = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Icon::WARNING;
        }
        if (isset($icon)) {
        ...
        foreach ($this->pending_stylesheets as $pending_stylesheet_index => &$pending_stylesheet) {
            ...
            foreach ($pending_stylesheet['tokens'] as $token) {
                ...
                foreach ($selectors_parsed as $selector => $parsed_selector) {
                    $should_include = $this->is_customize_preview || (empty($parsed_selector[self::SELECTOR_EXTRACTED_CLASSES]) || $this->has_used_class_name($parsed_selector[self::SELECTOR_EXTRACTED_CLASSES])) && (empty($parsed_selector[self::SELECTOR_EXTRACTED_IDS]) || 0 === \count(\array_filter($parsed_selector[self::SELECTOR_EXTRACTED_IDS], function ($id) {
