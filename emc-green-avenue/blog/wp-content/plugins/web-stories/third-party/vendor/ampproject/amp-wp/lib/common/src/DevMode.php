...
final class DevMode
{
    ...
    {
        if (!$document instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Document) {
            $document = \Google\Web_Stories_Dependencies\AmpProject\Dom\Document::fromNode($document);
        }
    ...
    private static function getDocument(\DOMNode $node)
    {
        ...
        if (!$document instanceof \Google\Web_Stories_Dependencies\AmpProject\Dom\Document) {
            $document = \Google\Web_Stories_Dependencies\AmpProject\Dom\Document::fromNode($node);
        }
        return $document;
    }
}
