...
final class AmpRuntimeCss implements \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Transformer
{
     ...
     */
    private function addStaticCss(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \DOMElement $ampRuntimeStyle, \Google\Web_Stories_Dependencies\AmpProject\Optimizer\ErrorCollection $errors)
    {
        $version = $this->configuration->get(\Google\Web_Stories_Dependencies\AmpProject\Optimizer\Configuration\AmpRuntimeCssConfiguration::VERSION);
    ...
    private function inlineCss(\DOMElement $ampRuntimeStyle, $version)
    {
        ...
        if (!empty($version)) {
            $v0CssUrl = $this->appendRuntimeVersion(\Google\Web_Stories_Dependencies\AmpProject\Amp::CACHE_HOST, $version) . '/' . self::V0_CSS;
        } else {
     ...
     * @param DOMElement $ampRuntimeStyle DOM element for the <style amp-runtime> tag to inline the CSS into.
     */
    private function linkCss(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $document, \DOMElement $ampRuntimeStyle)
    {
