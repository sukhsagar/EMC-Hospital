...
final class CssRule
{
    ...
    {
        $this->selectors = \array_values(\array_unique(\array_filter(\array_map([$this, 'normalizeSelector'], $this->separateSelectors($selectors)))));
        $this->properties = \array_values(\array_unique(\array_filter(\array_map([$this, 'normalizeProperty'], $this->separateProperties($properties)))));
    ...
    public function getCss()
    {
        if ($this->renderedCss === null) {
            ...
            $properties = \implode(';', \array_map(static function ($property) {
                return \trim($property, " \t\n\r\0\v;");
            }, $this->properties));
     ...
     */
    public function canBeMerged(\Google\Web_Stories_Dependencies\AmpProject\Optimizer\CssRule $that)
    {
        if ($this->mediaQuery !== $that->mediaQuery) {
            return \false;
        }
        if (\count($this->properties) !== \count($that->properties) || \array_diff($this->properties, $that->properties)) {
            return \false;
        }
        return \true;
    }
     ...
     */
    public function mergeWith(\Google\Web_Stories_Dependencies\AmpProject\Optimizer\CssRule $that)
    {
        $cssRule = new self(\array_merge($this->selectors, $that->selectors), \array_merge($this->properties, $that->properties));
