...
class AMP_Meta_Sanitizer extends \Google\Web_Stories_Dependencies\AMP_Base_Sanitizer
{
    ...
    public function sanitize()
    {
        ...
        foreach ($meta_elements as $meta_element) {
            ...
            if ($meta_element->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT)) {
                $meta_element->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT, \preg_replace('/\\s*=\\s*/', '=', $meta_element->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT)));
            }
            /**
             * Meta tag to process.
             *
             ...
             */
            if ($meta_element->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CHARSET)) {
                $this->meta_tags[self::TAG_CHARSET][] = $meta_element->parentNode->removeChild($meta_element);
            } elseif ($meta_element->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HTTP_EQUIV)) {
                $this->meta_tags[self::TAG_HTTP_EQUIV][] = $meta_element->parentNode->removeChild($meta_element);
            } elseif (\Google\Web_Stories_Dependencies\AmpProject\Attribute::VIEWPORT === $meta_element->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::NAME)) {
                $this->meta_tags[self::TAG_VIEWPORT][] = $meta_element->parentNode->removeChild($meta_element);
            } elseif (\Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_SCRIPT_SRC === $meta_element->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::NAME)) {
    ...
    protected function ensure_boilerplate_is_present()
    {
        ...
        if (!$style) {
            $style = $this->dom->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::STYLE);
            $style->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_BOILERPLATE, '');
            $style->appendChild($this->dom->createTextNode(\Google\Web_Stories_Dependencies\amp_get_boilerplate_stylesheets()[0]));
        } else {
        ...
        if (!$noscript) {
            $noscript = $this->dom->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::NOSCRIPT);
            $style = $this->dom->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::STYLE);
            $style->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_BOILERPLATE, '');
            $style->appendChild($this->dom->createTextNode(\Google\Web_Stories_Dependencies\amp_get_boilerplate_stylesheets()[1]));
    ...
    protected function process_amp_script_meta_tags()
    {
        ...
        $content_values = [$first_meta_amp_script_src->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT)];
        ...
        while (!empty($this->meta_tags[self::TAG_AMP_SCRIPT_SRC])) {
            ...
            $content_values[] = $meta_amp_script_src->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT);
        }
        $first_meta_amp_script_src->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT, \implode(' ', $content_values));
    ...
    {
        return \Google\Web_Stories_Dependencies\AMP_DOM_Utils::create_node($this->dom, \Google\Web_Stories_Dependencies\AmpProject\Tag::META, [\Google\Web_Stories_Dependencies\AmpProject\Attribute::NAME => \Google\Web_Stories_Dependencies\AmpProject\Attribute::VIEWPORT, \Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT => $viewport]);
    }
    /**
