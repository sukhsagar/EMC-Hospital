...
class AMP_Tag_And_Attribute_Sanitizer extends \Google\Web_Stories_Dependencies\AMP_Base_Sanitizer
{
    ...
    private function get_rule_spec_list_to_validate(\DOMElement $node, $rule_spec)
    {
        ...
        if (isset($rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::TAG_SPEC]['extension_spec'])) {
            ...
            $rule_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ATTR_SPEC_LIST]['src'] = [\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_REGEX => \implode('', [
                '^',
    ...
    private function process_node(\DOMElement $node)
    {
        ...
        if (isset($tag_spec['amp_layout'])) {
            ...
            if (isset($tag_spec['amp_layout']['supported_layouts'])) {
                $layouts = \wp_array_slice_assoc(\Google\Web_Stories_Dependencies\AmpProject\Layout::FROM_SPEC, $tag_spec['amp_layout']['supported_layouts']);
    ...
    private function get_required_script_components(\DOMElement $node, $tag_spec, $attr_spec_list)
    {
        ...
        if ($node instanceof \DOMElement && !\in_array('amp-bind', $this->script_components, \true)) {
            foreach ($node->attributes as $name => $value) {
                if (\Google\Web_Stories_Dependencies\AmpProject\Dom\Document::AMP_BIND_DATA_ATTR_PREFIX === \substr($name, 0, 14)) {
    ...
    {
        if (!empty($tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_PARENT]) && !$this->has_parent($node, $tag_spec[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::MANDATORY_PARENT])) {
    ...
    private function is_valid_layout($tag_spec, $node)
    {
        ...
        $layout_attr = $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::LAYOUT);
        $sizes_attr = $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::SIZES);
        $heights_attr = $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHTS);
        $allow_fluid = \Google\Web_Stories_Dependencies\AmpProject\Layout::FLUID === $layout_attr;
        $allow_auto = \true;
        $input_width = new \Google\Web_Stories_Dependencies\AmpProject\CssLength($node->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH) ? $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH) : null);
        $input_width->validate($allow_auto, $allow_fluid);
        if (!$input_width->isValid()) {
            return ['code' => self::INVALID_LAYOUT_WIDTH, 'attribute' => \Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH];
        }
        $input_height = new \Google\Web_Stories_Dependencies\AmpProject\CssLength($node->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT) ? $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT) : null);
        $input_height->validate($allow_auto, $allow_fluid);
        if (!$input_height->isValid()) {
            return ['code' => self::INVALID_LAYOUT_HEIGHT, 'attribute' => \Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT];
        }
        ...
        if (!$this->supports_layout($tag_spec, $layout)) {
             ...
             */
            if (!$layout_attr && \Google\Web_Stories_Dependencies\AmpProject\Layout::CONTAINER === $layout && $this->supports_layout($tag_spec, \Google\Web_Stories_Dependencies\AmpProject\Layout::RESPONSIVE)) {
        ...
        if ($height->isAuto() && \Google\Web_Stories_Dependencies\AmpProject\Layout::FLEX_ITEM !== $layout) {
            return ['code' => self::INVALID_LAYOUT_AUTO_HEIGHT, 'attribute' => \Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT];
        }
        // FIXED, FIXED_HEIGHT, INTRINSIC, RESPONSIVE must have height set.
        if (\in_array($layout, [\Google\Web_Stories_Dependencies\AmpProject\Layout::FIXED, \Google\Web_Stories_Dependencies\AmpProject\Layout::FIXED_HEIGHT, \Google\Web_Stories_Dependencies\AmpProject\Layout::INTRINSIC, \Google\Web_Stories_Dependencies\AmpProject\Layout::RESPONSIVE], \true) && !$height->isDefined()) {
            return ['code' => self::INVALID_LAYOUT_NO_HEIGHT, 'attribute' => \Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT];
        }
        // For FIXED_HEIGHT if width is set it must be auto.
        if (\Google\Web_Stories_Dependencies\AmpProject\Layout::FIXED_HEIGHT === $layout && $width->isDefined() && !$width->isAuto()) {
            return ['code' => self::INVALID_LAYOUT_FIXED_HEIGHT, 'attribute' => \Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH, 'required_attr_value' => \Google\Web_Stories_Dependencies\AmpProject\CssLength::AUTO];
        }
        // FIXED, INTRINSIC, RESPONSIVE must have width set and not be auto.
        if (\in_array($layout, [\Google\Web_Stories_Dependencies\AmpProject\Layout::FIXED, \Google\Web_Stories_Dependencies\AmpProject\Layout::INTRINSIC, \Google\Web_Stories_Dependencies\AmpProject\Layout::RESPONSIVE], \true)) {
            if (!$width->isDefined() || $width->isAuto()) {
                return ['code' => self::INVALID_LAYOUT_AUTO_WIDTH, 'attribute' => \Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH];
            }
        }
        // INTRINSIC, RESPONSIVE must have same units for height and width.
        if (\in_array($layout, [\Google\Web_Stories_Dependencies\AmpProject\Layout::INTRINSIC, \Google\Web_Stories_Dependencies\AmpProject\Layout::RESPONSIVE], \true) && $width->getUnit() !== $height->getUnit()) {
            return ['code' => self::INVALID_LAYOUT_UNIT_DIMENSIONS];
        }
        // Heights attribute is only allowed for RESPONSIVE layout.
        if (!$this->is_empty_attribute_value($heights_attr) && \Google\Web_Stories_Dependencies\AmpProject\Layout::RESPONSIVE !== $layout) {
            return ['code' => self::INVALID_LAYOUT_HEIGHTS, 'attribute' => \Google\Web_Stories_Dependencies\AmpProject\Attribute::LAYOUT, 'required_attr_value' => \Google\Web_Stories_Dependencies\AmpProject\Layout::RESPONSIVE];
        }
        return \true;
    }
    /**
    ...
    {
        if (!empty($this->open_elements[\Google\Web_Stories_Dependencies\AmpProject\Tag::TEMPLATE])) {
            while ($node->parentNode instanceof \DOMElement) {
                $node = $node->parentNode;
                if (\Google\Web_Stories_Dependencies\AmpProject\Tag::TEMPLATE === $node->nodeName && \Google\Web_Stories_Dependencies\AmpProject\Extension::MUSTACHE === $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::TYPE)) {
                    return \true;
                }
            }
        } elseif (!empty($this->open_elements[\Google\Web_Stories_Dependencies\AmpProject\Tag::SCRIPT])) {
            while ($node->parentNode instanceof \DOMElement) {
                $node = $node->parentNode;
                if (\Google\Web_Stories_Dependencies\AmpProject\Tag::SCRIPT === $node->nodeName && \Google\Web_Stories_Dependencies\AmpProject\Extension::MUSTACHE === $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::TEMPLATE) && \Google\Web_Stories_Dependencies\AmpProject\Attribute::TYPE_TEXT_PLAIN === $node->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::TYPE)) {
                    return \true;
                }
            }
        }
    ...
    {
        foreach ([\Google\Web_Stories_Dependencies\AmpProject\Attribute::LAYOUT, \Google\Web_Stories_Dependencies\AmpProject\Attribute::WIDTH, \Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHT, \Google\Web_Stories_Dependencies\AmpProject\Attribute::SIZES, \Google\Web_Stories_Dependencies\AmpProject\Attribute::HEIGHTS] as $attribute) {
     ...
     * @return CssLength
     */
    private function calculate_width($amp_layout_spec, $input_layout, \Google\Web_Stories_Dependencies\AmpProject\CssLength $input_width)
    {
        if ((!\array_key_exists($input_layout, \Google\Web_Stories_Dependencies\AmpProject\Layout::TO_SPEC) || \Google\Web_Stories_Dependencies\AmpProject\Layout::FIXED === $input_layout) && !$input_width->isDefined() && isset($amp_layout_spec['defines_default_width'])) {
            $css_length = new \Google\Web_Stories_Dependencies\AmpProject\CssLength('1px');
     ...
     * @return CssLength
     */
    private function calculate_height($amp_layout_spec, $input_layout, \Google\Web_Stories_Dependencies\AmpProject\CssLength $input_height)
    {
        if ((!\array_key_exists($input_layout, \Google\Web_Stories_Dependencies\AmpProject\Layout::TO_SPEC) || \in_array($input_layout, [\Google\Web_Stories_Dependencies\AmpProject\Layout::FIXED, \Google\Web_Stories_Dependencies\AmpProject\Layout::FIXED_HEIGHT], \true)) && !$input_height->isDefined() && isset($amp_layout_spec['defines_default_height'])) {
            $css_length = new \Google\Web_Stories_Dependencies\AmpProject\CssLength('1px');
            $css_length->validate(\false, \false);
            return $css_length;
        }
     ...
     */
    private function calculate_layout($layout_attr, \Google\Web_Stories_Dependencies\AmpProject\CssLength $width, \Google\Web_Stories_Dependencies\AmpProject\CssLength $height, $sizes_attr, $heights_attr)
    {
        if (!$this->is_empty_attribute_value($layout_attr)) {
            ...
            return \Google\Web_Stories_Dependencies\AmpProject\Layout::FLUID;
        ...
        if (isset($attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOWED_PROTOCOL])) {
            if ($node->hasAttribute($attr_name)) {
                foreach ($this->extract_attribute_urls($node->getAttributeNode($attr_name)) as $url) {
                    ...
                    if (isset($url_scheme) && !\in_array(\strtolower($url_scheme), $attr_spec_rule[\Google\Web_Stories_Dependencies\AMP_Rule_Spec::VALUE_URL][\Google\Web_Stories_Dependencies\AMP_Rule_Spec::ALLOWED_PROTOCOL], \true)) {
                        return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::FAIL;
                    }
                }
                return \Google\Web_Stories_Dependencies\AMP_Rule_Spec::PASS;
    ...
    private function is_amp_allowed_attribute(\DOMAttr $attr_node, $attr_spec_list)
    {
        ...
        if (isset($attr_spec_list[$attr_name]) || 'data-' === \substr($attr_name, 0, 5) && \Google\Web_Stories_Dependencies\AmpProject\Dom\Document::AMP_BIND_DATA_ATTR_PREFIX !== \substr($attr_name, 0, 14) || 'html' === $attr_node->parentNode->nodeName && \in_array($attr_node->nodeName, ['amp', '⚡'], \true)) {
            return \true;
        }
    ...
    private function replace_node_with_children(\DOMElement $node)
    {
        ...
        if (isset($this->should_not_replace_nodes[$node->nodeName]) && \in_array($node, $this->should_not_replace_nodes[$node->nodeName], \true)) {
            return;
        }
    ...
    private function supports_layout($tag_spec, $layout, $fallback = \false)
    {
        ...
        return \in_array(\Google\Web_Stories_Dependencies\AmpProject\Layout::TO_SPEC[$layout], $tag_spec['amp_layout']['supported_layouts'], \true);
    }
}
