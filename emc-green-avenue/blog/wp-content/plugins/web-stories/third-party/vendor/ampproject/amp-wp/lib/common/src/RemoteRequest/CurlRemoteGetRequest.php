 ...
 */
final class CurlRemoteGetRequest implements \Google\Web_Stories_Dependencies\AmpProject\RemoteGetRequest
{
    /**
     ...
     */
    public function __construct($sslVerify = \true, $timeout = self::DEFAULT_TIMEOUT, $retries = self::DEFAULT_RETRIES)
    {
            ...
            if ($body === \false) {
                ...
                if (!$retriesLeft || \in_array($curlErrno, self::RETRYABLE_ERROR_CODES, \true) === \false) {
                    if (!empty($status) && \is_numeric($status)) {
                        throw \Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToGetFromRemoteUrl::withHttpStatus($url, (int) $status);
                    }
                    throw \Google\Web_Stories_Dependencies\AmpProject\Exception\FailedToGetFromRemoteUrl::withoutHttpStatus($url);
                }
                continue;
            }
            return new \Google\Web_Stories_Dependencies\AmpProject\RemoteRequest\RemoteGetRequestResponse($body, $headers, (int) $status);
        } while ($retriesLeft--);
        // This should never be triggered, but we want to ensure we always have a typed return value,
        // to make PHPStan happy.
        return new \Google\Web_Stories_Dependencies\AmpProject\RemoteRequest\RemoteGetRequestResponse('', [], 500);
    }
}
