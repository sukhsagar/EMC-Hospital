...
final class Document extends \DOMDocument
{
     ...
     * @var string
     */
    const AMP_BIND_START_TAGS_PATTERN = '#<' . '(?P<name>[a-zA-Z0-9_\\-]+)' . '(?P<attrs>\\s' . '(?>[^>"\'\\[\\]]+|"[^"]*+"|\'[^\']*+\')*+' . '\\[[a-zA-Z0-9_\\-]+\\]' . '(?>[^>"\']+|"[^"]*+"|\'[^\']*+\')*+' . ')>#s';
    /*
     ...
     * @var string
     */
    const AMP_EMOJI_ATTRIBUTE_PATTERN = '/(<html\\s[^>]*?)(' . \Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_EMOJI_ALT . '|' . \Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_EMOJI . ')([^\\s^>]*)/iu';
    ...
    public static function fromNode(\DOMNode &$node)
    {
        ...
        $dom->hasInitialAmpDevMode = $dom->documentElement->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\DevMode::DEV_MODE_ATTRIBUTE);
        return $dom;
    }
    /**
    ...
    public function loadHTML($source, $options = 0)
    {
        ...
        if ($success) {
            ...
            $this->deduplicateTag(\Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD);
            $this->deduplicateTag(\Google\Web_Stories_Dependencies\AmpProject\Tag::BODY);
    ...
    public function loadHTMLFragment($source, $options = 0)
    {
        ...
        $libxml_previous_state = \libxml_use_internal_errors(\true);
        ...
        if ($success) {
            ...
            if ($meta instanceof \DOMElement && \Google\Web_Stories_Dependencies\AmpProject\Tag::META === $meta->tagName && self::HTML_HTTP_EQUIV_VALUE === $meta->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HTTP_EQUIV) && self::HTML_HTTP_EQUIV_CONTENT_VALUE === $meta->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT)) {
                $this->head->removeChild($meta);
            }
            $this->hasInitialAmpDevMode = $this->documentElement->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\DevMode::DEV_MODE_ATTRIBUTE);
        }
        return $success;
    }
    /**
    ...
    public function saveHTMLFragment(\DOMNode $node = null)
    {
        ...
        // Force-add http-equiv charset to make DOMDocument behave as it should.
        // See: http://php.net/manual/en/domdocument.loadhtml.php#78243.
        $charset = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::META);
        $charset->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HTTP_EQUIV, self::HTML_HTTP_EQUIV_VALUE);
        $charset->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CONTENT, self::HTML_HTTP_EQUIV_CONTENT_VALUE);
    ...
    private function insertMissingCharset()
    {
        ...
        $charset = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::META);
        $charset->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::CHARSET, self::AMP_ENCODING);
        $this->head->insertBefore($charset, $this->head->firstChild);
    }
    /**
    ...
    public function normalizeDomStructure()
    {
        if (!$this->documentElement) {
            $this->appendChild($this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::HTML));
        } elseif (\Google\Web_Stories_Dependencies\AmpProject\Tag::HTML !== $this->documentElement->nodeName) {
            ...
            $html = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::HTML);
            $this->insertBefore($html, $nextSibling);
            if ($oldDocumentElement->nodeName === \Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD) {
                $this->head = $oldDocumentElement;
            } else {
                $this->head = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD)->item(0);
                if (!$this->head) {
                    $this->head = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD);
                }
            }
            $html->appendChild($this->head);
            if ($oldDocumentElement->nodeName === \Google\Web_Stories_Dependencies\AmpProject\Tag::BODY) {
                $this->body = $oldDocumentElement;
            } else {
                $this->body = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::BODY)->item(0);
                if (!$this->body) {
                    $this->body = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::BODY);
                }
            }
            ...
            $head = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD)->item(0);
            if (!$head) {
                $this->head = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD);
                $this->documentElement->insertBefore($this->head, $this->documentElement->firstChild);
            }
            $body = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::BODY)->item(0);
            if (!$body) {
                $this->body = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::BODY);
    ...
    {
        if (!$this->head->hasAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::PROFILE)) {
            return;
        }
        $profile = $this->head->getAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::PROFILE);
        if ($profile) {
            $link = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::LINK);
            $link->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::REL, \Google\Web_Stories_Dependencies\AmpProject\Attribute::PROFILE);
            $link->setAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::HREF, $profile);
            $this->head->appendChild($link);
        }
        $this->head->removeAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::PROFILE);
    }
    /**
    ...
    private function movePostBodyNodesToBody()
    {
        ...
        while ($this->documentElement->nextSibling) {
            $nextSibling = $this->documentElement->nextSibling;
            if ($nextSibling instanceof \DOMElement && \Google\Web_Stories_Dependencies\AmpProject\Tag::HTML === $nextSibling->nodeName) {
    ...
    private function replaceSelfClosingTags($html)
    {
        ...
        if (null === $regexPattern) {
            $regexPattern = '#<(' . \implode('|', \Google\Web_Stories_Dependencies\AmpProject\Tag::SELF_CLOSING_TAGS) . ')([^>]*?)(?>\\s*\\/)?>(?!</\\1>)#';
        }
    ...
    private function restoreSelfClosingTags($html)
    {
        ...
        if (null === $regexPattern) {
            $regexPattern = '#</(' . \implode('|', \Google\Web_Stories_Dependencies\AmpProject\Tag::SELF_CLOSING_TAGS) . ')>#i';
        }
    ...
    private function maybeRestoreNoscriptElements()
    {
        foreach ($this->noscriptPlaceholderComments as $id => $noscriptHtmlFragment) {
            ...
            $exportBody = $noscriptFragmentDocument->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::BODY)->item(0);
            if (!$exportBody) {
                continue;
            }
            ...
            while ($exportBody->firstChild) {
                ...
                $importNode = $this->importNode($importNode, \true);
    ...
    private function restoreMustacheScriptTemplates()
    {
        ...
        foreach ($tmp_script_elements as $tmp_script_element) {
            $script = $this->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::SCRIPT);
    ...
    private function convertAmpBindAttributes($html)
    {
        ...
        $replaceCallback = static function ($tagMatches) {
            ...
            while (\preg_match(self::AMP_BIND_ATTR_PATTERN, \substr($oldAttrs, $offset), $attrMatches)) {
                ...
                if ('[' === $attrMatches['name'][0]) {
                    $newAttrs .= ' ' . self::AMP_BIND_DATA_ATTR_PREFIX . \trim($attrMatches['name'], '[]');
        ...
        $converted = \preg_replace_callback(self::AMP_BIND_START_TAGS_PATTERN, $replaceCallback, $html);
        /*
    ...
    private function adaptEncoding($source)
    {
        ...
        if (self::UNKNOWN_ENCODING === $this->originalEncoding && \function_exists('mb_detect_encoding')) {
            $this->originalEncoding = \mb_detect_encoding($source, self::ENCODING_DETECTION_ORDER, \true);
        }
        ...
        $target = \false;
        if (self::AMP_ENCODING !== \strtolower($this->originalEncoding)) {
            $target = \function_exists('mb_convert_encoding') ? \mb_convert_encoding($source, self::AMP_ENCODING, $this->originalEncoding) : \false;
        }
    ...
    private function detectAndStripEncoding($content)
    {
        ...
        foreach ($this->findTags($content, \Google\Web_Stories_Dependencies\AmpProject\Tag::META, \Google\Web_Stories_Dependencies\AmpProject\Attribute::HTTP_EQUIV) as $potentialHttpEquivTag) {
            $encoding = $this->extractValue($potentialHttpEquivTag, \Google\Web_Stories_Dependencies\AmpProject\Attribute::CHARSET);
            if (\false !== $encoding) {
        ...
        $charsetTag = $this->findTag($content, \Google\Web_Stories_Dependencies\AmpProject\Tag::META, \Google\Web_Stories_Dependencies\AmpProject\Attribute::CHARSET);
        if ($charsetTag) {
            $encoding = $this->extractValue($charsetTag, \Google\Web_Stories_Dependencies\AmpProject\Attribute::CHARSET);
    ...
    private function sanitizeEncoding($encoding)
    {
        ...
        if (!\in_array($lcEncoding, $knownEncodings, \true)) {
    ...
    private function replaceMustacheTemplateTokens()
    {
        ...
        foreach ($templates as $template) {
            foreach ($this->xpath->query(self::XPATH_URL_ENCODED_ATTRIBUTES_QUERY, $template) as $attribute) {
                $attribute->nodeValue = \str_replace(\array_keys($mustacheTagPlaceholders), $mustacheTagPlaceholders, $attribute->nodeValue, $count);
                if ($count) {
    ...
    private function restoreAmpEmojiAttribute($html)
    {
        ...
        return \preg_replace('/(<html\\s[^>]*?)' . \preg_quote(self::EMOJI_AMP_ATTRIBUTE_PLACEHOLDER, '/') . '="([^"]*)"/i', '\\1' . $this->usedAmpEmoji . '\\2', $html, 1);
    }
    /**
    ...
    public function deduplicateTag($tagName)
    {
        ...
        if (\in_array($tagName, [\Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD, \Google\Web_Stories_Dependencies\AmpProject\Tag::BODY], \true)) {
    ...
    {
        return $node instanceof \DOMElement && \in_array($node->nodeName, \Google\Web_Stories_Dependencies\AmpProject\Tag::ELEMENTS_ALLOWED_IN_HEAD, \true) || $node instanceof \DOMText && \preg_match('/^\\s*$/', $node->nodeValue) || $node instanceof \DOMComment;
    }
    /**
    ...
    public function __get($name)
    {
        switch ($name) {
            case 'xpath':
                ...
                return $this->xpath;
            case \Google\Web_Stories_Dependencies\AmpProject\Tag::HTML:
                $this->html = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::HTML)->item(0);
                if (null === $this->html) {
                ...
                return $this->html;
            case \Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD:
                $this->head = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::HEAD)->item(0);
                if (null === $this->head) {
                    // Document was assembled manually and bypassed normalisation.
                    $this->normalizeDomStructure();
                ...
                return $this->head;
            case \Google\Web_Stories_Dependencies\AmpProject\Tag::BODY:
                $this->body = $this->getElementsByTagName(\Google\Web_Stories_Dependencies\AmpProject\Tag::BODY)->item(0);
                if (null === $this->body) {
                    // Document was assembled manually and bypassed normalisation.
                    $this->normalizeDomStructure();
