...
class AMP_Validation_Error_Taxonomy
{
    ...
    {
        $dev_tools_user_access = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Services::get('dev_tools.user_access');
        ...
        \Google\Web_Stories_Dependencies\register_taxonomy(self::TAXONOMY_SLUG, \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, [
            'labels' => [
                'name' => \Google\Web_Stories_Dependencies\_x('AMP Validation Error Index', 'taxonomy general name', 'amp'),
                'singular_name' => \Google\Web_Stories_Dependencies\_x('AMP Validation Error', 'taxonomy singular name', 'amp'),
        ...
        if (\Google\Web_Stories_Dependencies\is_admin()) {
            self::add_admin_hooks();
        }
        \Google\Web_Stories_Dependencies\add_action('created_' . self::TAXONOMY_SLUG, [__CLASS__, 'clear_cached_counts']);
        \Google\Web_Stories_Dependencies\add_action('edited_' . self::TAXONOMY_SLUG, [__CLASS__, 'clear_cached_counts']);
        \Google\Web_Stories_Dependencies\add_action('delete_' . self::TAXONOMY_SLUG, [__CLASS__, 'clear_cached_counts']);
    }
    /**
    ...
    public static function get_term($error)
    {
        ...
        return \Google\Web_Stories_Dependencies\get_term_by('slug', $slug, self::TAXONOMY_SLUG);
    }
    /**
    ...
    public static function delete_empty_terms()
    {
        ...
        \Google\Web_Stories_Dependencies\wp_update_term_count_now($empty_term_ids, self::TAXONOMY_SLUG);
        $deleted_count = 0;
    ...
    {
        $term = \Google\Web_Stories_Dependencies\get_term((int) $term_id, self::TAXONOMY_SLUG);
        // Skip if the term count was not actually 0.
        if (!$term || 0 !== $term->count) {
            return \false;
        }
        if (\true === \Google\Web_Stories_Dependencies\wp_delete_term($term->term_id, self::TAXONOMY_SLUG)) {
            return \true;
        }
        return \false;
    }
    ...
    public static function sanitize_term_status($status, $options = [])
    {
        ...
        if (\is_string($status)) {
            $statuses = \Google\Web_Stories_Dependencies\wp_parse_id_list($status);
        } else {
    ...
    public static function get_validation_error_count($args = [])
    {
        ...
        $term_count = \Google\Web_Stories_Dependencies\wp_count_terms(self::TAXONOMY_SLUG, $args);
        self::$should_filter_terms_clauses_for_error_validation_status = \true;
        if (isset($args['group'])) {
            \Google\Web_Stories_Dependencies\remove_filter('terms_clauses', $filter);
        }
        $result = (int) $term_count;
        $cached_counts[$cache_key] = $result;
        \set_transient(self::TRANSIENT_KEY_ERROR_INDEX_COUNTS, $cached_counts, \Google\Web_Stories_Dependencies\DAY_IN_SECONDS);
        return $result;
    }
    /**
     ...
     */
    public static function filter_posts_where_for_validation_error_status($where, \Google\Web_Stories_Dependencies\WP_Query $query)
    {
        global $wpdb;
        ...
        $error_type = \Google\Web_Stories_Dependencies\sanitize_key($query->get(self::VALIDATION_ERROR_TYPE_QUERY_VAR));
        /*
    ...
    public static function add_admin_hooks()
    {
        ...
        \Google\Web_Stories_Dependencies\add_action('load-edit-tags.php', [__CLASS__, 'add_group_terms_clauses_filter']);
        \Google\Web_Stories_Dependencies\add_action('load-edit-tags.php', [__CLASS__, 'add_error_type_clauses_filter']);
        \Google\Web_Stories_Dependencies\add_action('load-post.php', [__CLASS__, 'add_error_type_clauses_filter']);
        \Google\Web_Stories_Dependencies\add_action('load-edit-tags.php', [__CLASS__, 'add_order_clauses_from_description_json']);
        \Google\Web_Stories_Dependencies\add_action('load-post.php', [__CLASS__, 'add_order_clauses_from_description_json']);
        \Google\Web_Stories_Dependencies\add_action(\sprintf('after-%s-table', self::TAXONOMY_SLUG), [__CLASS__, 'render_taxonomy_filters']);
        \Google\Web_Stories_Dependencies\add_action(\sprintf('after-%s-table', self::TAXONOMY_SLUG), [__CLASS__, 'render_link_to_invalid_urls_screen']);
        \add_filter('terms_clauses', [__CLASS__, 'filter_terms_clauses_for_description_search'], 10, 3);
        \Google\Web_Stories_Dependencies\add_action('admin_notices', [__CLASS__, 'add_admin_notices']);
        \add_filter(self::TAXONOMY_SLUG . '_row_actions', [__CLASS__, 'filter_tag_row_actions'], \PHP_INT_MAX, 2);
        if (\Google\Web_Stories_Dependencies\get_taxonomy(self::TAXONOMY_SLUG)->show_in_menu) {
            \Google\Web_Stories_Dependencies\add_action('admin_menu', [__CLASS__, 'add_admin_menu_validation_error_item']);
        }
        ...
        \Google\Web_Stories_Dependencies\add_action('load-edit-tags.php', [__CLASS__, 'handle_inline_edit_request']);
        \Google\Web_Stories_Dependencies\add_action('load-edit-tags.php', [__CLASS__, 'handle_clear_empty_terms_request']);
        \Google\Web_Stories_Dependencies\add_action('load-edit.php', [__CLASS__, 'handle_inline_edit_request']);
        ...
        \Google\Web_Stories_Dependencies\add_action('admin_enqueue_scripts', static function () {
            $current_screen = \Google\Web_Stories_Dependencies\get_current_screen();
            if (!$current_screen) {
                return;
            }
            if (\Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG === $current_screen->taxonomy) {
                \Google\Web_Stories_Dependencies\wp_add_inline_style('common', '
				...
				');
                \Google\Web_Stories_Dependencies\wp_register_style('amp-validation-tooltips', \Google\Web_Stories_Dependencies\amp_get_asset_url('css/amp-validation-tooltips.css'), ['wp-pointer'], \Google\Web_Stories_Dependencies\AMP__VERSION);
                \wp_styles()->add_data('amp-validation-tooltips', 'rtl', 'replace');
                $asset_file = \Google\Web_Stories_Dependencies\AMP__DIR__ . '/assets/js/amp-validation-tooltips.asset.php';
                $asset = (require $asset_file);
                $dependencies = $asset['dependencies'];
                $version = $asset['version'];
                \Google\Web_Stories_Dependencies\wp_register_script('amp-validation-tooltips', \Google\Web_Stories_Dependencies\amp_get_asset_url('js/amp-validation-tooltips.js'), $dependencies, $version, \true);
                \Google\Web_Stories_Dependencies\wp_enqueue_style('amp-validation-error-taxonomy', \Google\Web_Stories_Dependencies\amp_get_asset_url('css/amp-validation-error-taxonomy.css'), ['common', 'amp-validation-tooltips', 'amp-icons'], \Google\Web_Stories_Dependencies\AMP__VERSION);
                \wp_styles()->add_data('amp-validation-error-taxonomy', 'rtl', 'replace');
                \Google\Web_Stories_Dependencies\wp_enqueue_script('amp-validation-detail-toggle', \Google\Web_Stories_Dependencies\amp_get_asset_url('js/amp-validation-detail-toggle.js'), ['wp-dom-ready', 'wp-i18n', 'amp-validation-tooltips'], \Google\Web_Stories_Dependencies\AMP__VERSION, \true);
            }
            if ('post' === $current_screen->base && \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG === $current_screen->post_type) {
                \Google\Web_Stories_Dependencies\wp_enqueue_style('amp-validation-single-error-url', \Google\Web_Stories_Dependencies\amp_get_asset_url('css/amp-validation-single-error-url.css'), ['common', 'amp-icons'], \Google\Web_Stories_Dependencies\AMP__VERSION);
                \wp_styles()->add_data('amp-validation-single-error-url', 'rtl', 'replace');
                $asset_file = \Google\Web_Stories_Dependencies\AMP__DIR__ . '/assets/js/amp-validation-single-error-url-details.asset.php';
                $asset = (require $asset_file);
                $dependencies = $asset['dependencies'];
                $version = $asset['version'];
                \Google\Web_Stories_Dependencies\wp_enqueue_script('amp-validation-single-error-url-details', \Google\Web_Stories_Dependencies\amp_get_asset_url('js/amp-validation-single-error-url-details.js'), $dependencies, $version, \true);
            }
        });
        ...
        if (\Google\Web_Stories_Dependencies\current_user_can('manage_options')) {
            \add_filter('parent_file', static function ($parent_file) {
                if (\Google\Web_Stories_Dependencies\get_current_screen()->taxonomy === \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG) {
        ...
        \add_filter('list_table_primary_column', static function ($primary_column) {
            if (\Google\Web_Stories_Dependencies\get_current_screen() && \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::TAXONOMY_SLUG === \Google\Web_Stories_Dependencies\get_current_screen()->taxonomy) {
                $primary_column = 'error_code';
            }
            return $primary_column;
        });
        // Jump to the requested line when opening the file editor.
        \Google\Web_Stories_Dependencies\add_action('admin_enqueue_scripts', function ($hook_suffix) {
            if (!isset($_GET['line'])) {
            ...
            if ('plugin-editor.php' === $hook_suffix || 'theme-editor.php' === $hook_suffix) {
                \Google\Web_Stories_Dependencies\wp_add_inline_script('wp-theme-plugin-editor', \sprintf('
								(
    ...
    public static function add_term_filter_query_var($url, $tax)
    {
        ...
        if (isset($_POST[self::VALIDATION_ERROR_TYPE_QUERY_VAR]) && \in_array(
            ...
            $url = \add_query_arg(
                ...
                \Google\Web_Stories_Dependencies\sanitize_key(\Google\Web_Stories_Dependencies\wp_unslash($_POST[self::VALIDATION_ERROR_TYPE_QUERY_VAR])),
                // phpcs:ignore WordPress.Security.NonceVerification.Missing
                $url
            );
        }
    ...
    {
        if (self::TAXONOMY_SLUG !== \Google\Web_Stories_Dependencies\get_current_screen()->taxonomy || !isset($_GET[self::VALIDATION_ERROR_STATUS_QUERY_VAR])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        self::$should_filter_terms_clauses_for_error_validation_status = \true;
    ...
    {
        if (self::TAXONOMY_SLUG !== \Google\Web_Stories_Dependencies\get_current_screen()->taxonomy || !isset($_GET[self::VALIDATION_ERROR_TYPE_QUERY_VAR])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        $type = \Google\Web_Stories_Dependencies\sanitize_key(\Google\Web_Stories_Dependencies\wp_unslash($_GET[self::VALIDATION_ERROR_TYPE_QUERY_VAR]));
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    ...
    {
        if (self::TAXONOMY_SLUG !== \Google\Web_Stories_Dependencies\get_current_screen()->taxonomy) {
            return;
        }
    ...
    public static function render_taxonomy_filters($taxonomy_name)
    {
        ...
        \Google\Web_Stories_Dependencies\submit_button(\__('Apply Filter', 'amp'), '', 'filter_action', \false, ['id' => 'doaction']);
    ...
    public static function render_link_to_invalid_urls_screen($taxonomy_name)
    {
        ...
        if (!\Google\Web_Stories_Dependencies\current_user_can($post_type_object->cap->edit_posts)) {
            return;
        }
        $id = 'link-errors-url';
        \printf('<a href="%s" class="page-title-action" id="%s" hidden style="margin-left: 1rem;">%s</a>', \esc_url(\add_query_arg('post_type', \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, \Google\Web_Stories_Dependencies\admin_url('edit.php'))), \esc_attr($id), \esc_html__('View Validated URLs', 'amp'));
        ?>
		<script>
			jQuery( function( $ ) {
    ...
    {
        global $wp_query;
        $screen_base = \Google\Web_Stories_Dependencies\get_current_screen()->base;
        if ('edit-tags' === $screen_base) {
            ...
            $error_type = \Google\Web_Stories_Dependencies\sanitize_key($wp_query->get(self::VALIDATION_ERROR_TYPE_QUERY_VAR));
            if ($error_type && \in_array($error_type, self::get_error_types(), \true)) {
                $args[self::VALIDATION_ERROR_TYPE_QUERY_VAR] = $error_type;
            }
            $with_new_query = new \Google\Web_Stories_Dependencies\WP_Query(\array_merge($args, [self::VALIDATION_ERROR_STATUS_QUERY_VAR => [self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS, self::VALIDATION_ERROR_NEW_REJECTED_STATUS]]));
            $unreviewed_count = $with_new_query->found_posts;
            $with_rejected_query = new \Google\Web_Stories_Dependencies\WP_Query(\array_merge($args, [self::VALIDATION_ERROR_STATUS_QUERY_VAR => [self::VALIDATION_ERROR_NEW_REJECTED_STATUS, self::VALIDATION_ERROR_ACK_REJECTED_STATUS]]));
            $rejected_term_count = $with_rejected_query->found_posts;
            $with_accepted_query = new \Google\Web_Stories_Dependencies\WP_Query(\array_merge($args, [self::VALIDATION_ERROR_STATUS_QUERY_VAR => [self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS, self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS]]));
        ...
        ?></option>
			<?php 
        $options_config = [['text' => 'edit' === $screen_base ? \Google\Web_Stories_Dependencies\_x('With unreviewed errors', 'terms', 'amp') : \Google\Web_Stories_Dependencies\_x('Unreviewed errors', 'terms', 'amp'), 'value' => self::VALIDATION_ERROR_NEW_REJECTED_STATUS . ',' . self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS, 'error_count' => $unreviewed_count], ['text' => 'edit' === $screen_base ? \Google\Web_Stories_Dependencies\_x('With removed markup', 'terms', 'amp') : \Google\Web_Stories_Dependencies\_x('Removed markup', 'terms', 'amp'), 'value' => self::VALIDATION_ERROR_NEW_ACCEPTED_STATUS . ',' . self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS, 'error_count' => $accepted_term_count], ['text' => 'edit' === $screen_base ? \Google\Web_Stories_Dependencies\_x('With kept markup', 'terms', 'amp') : \Google\Web_Stories_Dependencies\_x('Kept markup', 'terms', 'amp'), 'value' => self::VALIDATION_ERROR_NEW_REJECTED_STATUS . ',' . self::VALIDATION_ERROR_ACK_REJECTED_STATUS, 'error_count' => $rejected_term_count]];
    ...
    {
        \printf('<option value="%s" %s>%s <span class="count">(%s)</span></option>', \esc_attr($option_value), \Google\Web_Stories_Dependencies\selected($selected_value, $option_value, \false), \esc_html($option_text), \esc_html(\Google\Web_Stories_Dependencies\number_format_i18n($error_count)));
    }
    /**
    ...
    {
        $error_type_filter_value = isset($_GET[self::VALIDATION_ERROR_TYPE_QUERY_VAR]) ? \Google\Web_Stories_Dependencies\sanitize_key(\Google\Web_Stories_Dependencies\wp_unslash($_GET[self::VALIDATION_ERROR_TYPE_QUERY_VAR])) : '';
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        /*
        ...
        ?>" <?php 
        \Google\Web_Stories_Dependencies\selected($error_type_filter_value, self::HTML_ELEMENT_ERROR_TYPE);
        ?>>
				<?php 
        if ('edit' === $screen_base) {
            ?>
        ...
        ?>" <?php 
        \Google\Web_Stories_Dependencies\selected($error_type_filter_value, self::HTML_ATTRIBUTE_ERROR_TYPE);
        ?>>
				<?php 
        if ('edit' === $screen_base) {
            ?>
        ...
        ?>" <?php 
        \Google\Web_Stories_Dependencies\selected($error_type_filter_value, self::JS_ERROR_TYPE);
        ?>>
				<?php 
        if ('edit' === $screen_base) {
            ?>
        ...
        ?>" <?php 
        \Google\Web_Stories_Dependencies\selected($error_type_filter_value, self::CSS_ERROR_TYPE);
        ?>>
				<?php 
        if ('edit' === $screen_base) {
            ?>
    ...
    public static function render_clear_empty_button()
    {
        ...
        if ($count > 0) {
            ...
            \Google\Web_Stories_Dependencies\submit_button(\__('Clear Empty', 'amp'), '', self::VALIDATION_ERROR_CLEAR_EMPTY_ACTION, \false);
        }
    }
    /**
    ...
    {
        if (!(self::TAXONOMY_SLUG === \Google\Web_Stories_Dependencies\get_current_screen()->taxonomy || \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG === \Google\Web_Stories_Dependencies\get_current_screen()->post_type)) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        ...
        if (!empty($_GET['amp_actioned']) && !empty($_GET['amp_actioned_count'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            $actioned = \Google\Web_Stories_Dependencies\sanitize_key($_GET['amp_actioned']);
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            ...
            if ('delete' === $actioned) {
                $message = \sprintf(
                    ...
                    \Google\Web_Stories_Dependencies\_n('Deleted %s instance of validation errors.', 'Deleted %s instances of validation errors.', \Google\Web_Stories_Dependencies\number_format_i18n($count), 'amp'),
                    $count
                );
            }
            if ($message) {
        ...
        if (isset($_GET[self::VALIDATION_ERRORS_CLEARED_QUERY_VAR])) {
            ...
            \printf('<div class="notice notice-success is-dismissible"><p>%s</p></div>', \esc_html(\sprintf(
                ...
                \Google\Web_Stories_Dependencies\_n('Cleared %s validation error for invalid markup that no longer occurs on the site.', 'Cleared %s validation errors for invalid markup that no longer occur on the site.', $cleared_count, 'amp'),
                \Google\Web_Stories_Dependencies\number_format_i18n($cleared_count)
            )));
        }
    }
    /**
     ...
     */
    public static function filter_tag_row_actions($actions, \Google\Web_Stories_Dependencies\WP_Term $tag)
    {
        global $pagenow;
        $term_id = $tag->term_id;
        $term = \Google\Web_Stories_Dependencies\get_term($term_id);
    ...
    public static function add_admin_menu_validation_error_item()
    {
        ...
        global $submenu;
        if (\Google\Web_Stories_Dependencies\current_user_can('manage_options')) {
            $taxonomy_caps = (object) \Google\Web_Stories_Dependencies\get_taxonomy(self::TAXONOMY_SLUG)->cap;
            // Yes, cap is an object not an array.
            \Google\Web_Stories_Dependencies\add_submenu_page(
        ...
        switch ($column_name) {
            case 'error_code':
                if ('post.php' === $pagenow) {
                    ...
                    $content .= \Google\Web_Stories_Dependencies\wp_kses_post(self::get_error_title_from_code($validation_error));
                } else {
                    $content .= '<p>';
                    $content .= \sprintf('<a href="%s">%s', \Google\Web_Stories_Dependencies\admin_url(\add_query_arg([self::TAXONOMY_SLUG => $term->name, 'post_type' => \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG], 'edit.php')), \Google\Web_Stories_Dependencies\wp_kses_post(self::get_error_title_from_code($validation_error)));
                }
                if ('post.php' === $pagenow) {
                    $content .= '</button>';
                } else {
            ...
            case 'status':
                ...
                if ('post.php' === $pagenow) {
                    $valid_color = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Icon::valid()->get_color();
                    $invalid_color = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Icon::invalid()->get_color();
                    ...
                    echo \esc_attr($status_select_name);
                    ?>" style="<?php 
                    echo \esc_attr($status_border_color);
                    ?>">
							<option value="<?php 
                    echo \esc_attr(self::VALIDATION_ERROR_ACK_ACCEPTED_STATUS);
                    ?>" <?php 
                    \Google\Web_Stories_Dependencies\selected($is_removed);
                    ?> data-color="<?php 
                    echo \esc_attr($valid_color);
                    ?>">
								<?php 
            ...
            case 'created_date_gmt':
                ...
                $created_date_gmt = \Google\Web_Stories_Dependencies\get_term_meta($term_id, 'created_date_gmt', \true);
                if ($created_date_gmt) {
                    try {
                        $created_datetime = new \DateTime($created_date_gmt, new \DateTimeZone('UTC'));
                        $timezone_string = \Google\Web_Stories_Dependencies\get_option('timezone_string');
                        if (!$timezone_string && \Google\Web_Stories_Dependencies\get_option('gmt_offset')) {
                            $timezone_string = \timezone_name_from_abbr('', \Google\Web_Stories_Dependencies\get_option('gmt_offset') * \Google\Web_Stories_Dependencies\HOUR_IN_SECONDS, \false);
                        }
                        if ($timezone_string) {
                            $created_datetime->setTimezone(new \DateTimeZone(\Google\Web_Stories_Dependencies\get_option('timezone_string')));
                        }
                    } catch (\Exception $e) {
                        unset($e);
                    }
                ...
                if (!$created_datetime) {
                    ...
                    $time_ago = \sprintf(
                        ...
                        \esc_html(\sprintf(\__('%s ago', 'amp'), \Google\Web_Stories_Dependencies\human_time_diff($created_datetime->getTimestamp())))
                    );
                }
                if ($created_datetime) {
            ...
            case 'reviewed':
                if ('post.php' === $pagenow) {
                    $checked = \Google\Web_Stories_Dependencies\checked(0 < ((int) $term->term_group & self::ACKNOWLEDGED_VALIDATION_ERROR_BIT_MASK), \true, \false);
    ...
    public static function render_single_url_error_details($validation_error, $term, $wrap_with_details = \true, $with_summary = \true)
    {
        ...
        if (isset($validation_error['type']) && self::JS_ERROR_TYPE === $validation_error['type']) {
            ?>
						<?php 
            echo \Google\Web_Stories_Dependencies\wp_kses_post(\sprintf(
        ...
        ?>
				...
				<p>
					<?php 
        echo \Google\Web_Stories_Dependencies\wp_kses_post(\__('If all invalid markup is &#8220;removed&#8221; the page will be served as AMP. However, the impact that the removal has on the page must be assessed to determine if the result is acceptable. If any invalid markup is &#8220;kept&#8221; then the page will not be served as AMP.', 'amp'));
        ?>
				</p>
			</dd>
    ...
    private static function get_file_editor_url($source)
    {
        ...
        if (null !== $editor_url_template && \false !== \strpos($editor_url_template, '{{file}}')) {
            ...
            if ('core' === $source['type']) {
                ...
                $file_path = \Google\Web_Stories_Dependencies\WP_PLUGIN_DIR . '/' . $source['name'];
                if ($source['name'] !== $source['file']) {
                    $file_path .= '/' . $source['file'];
                }
            } elseif ('mu-plugin' === $source['type']) {
                $file_path = \Google\Web_Stories_Dependencies\WPMU_PLUGIN_DIR . '/' . $source['name'];
            } elseif ('theme' === $source['type']) {
                $theme = \Google\Web_Stories_Dependencies\wp_get_theme($source['name']);
                if ($theme instanceof \Google\Web_Stories_Dependencies\WP_Theme && !$theme->errors()) {
        ...
        if (!$edit_url) {
            if ('plugin' === $source['type'] && \Google\Web_Stories_Dependencies\current_user_can('edit_plugins')) {
                ...
                if ($plugin) {
                    ...
                    $edit_url = \add_query_arg(['plugin' => \rawurlencode($plugin['file']), 'file' => \rawurlencode($file), 'line' => \rawurlencode($source['line'])], \Google\Web_Stories_Dependencies\admin_url('plugin-editor.php'));
                }
            } elseif ('theme' === $source['type'] && \Google\Web_Stories_Dependencies\current_user_can('edit_themes')) {
                $edit_url = \add_query_arg(['file' => \rawurlencode($source['file']), 'theme' => \rawurlencode($source['name']), 'line' => \rawurlencode($source['line'])], \Google\Web_Stories_Dependencies\admin_url('theme-editor.php'));
            }
        }
        return $edit_url;
    }
    ...
    private static function render_source_name($name, $type)
    {
        ...
        switch ($type) {
            case 'theme':
                $theme = \Google\Web_Stories_Dependencies\wp_get_theme($name);
                if (!$theme->errors()) {
    ...
    public static function render_sources($sources)
    {
        ...
        echo \esc_html(\sprintf(
            ...
            \Google\Web_Stories_Dependencies\_n('Source stack (%s)', 'Source stack (%s)', $source_count, 'amp'),
    ...
    private static function render_code_details($text)
    {
        ...
        echo \esc_html(\sprintf(
            ...
            \Google\Web_Stories_Dependencies\_n('%s byte', '%s bytes', $length, 'amp'),
    ...
    public static function handle_inline_edit_request()
    {
        ...
        if (self::TAXONOMY_SLUG !== \Google\Web_Stories_Dependencies\get_current_screen()->taxonomy && !isset($_GET['post_type'])) {
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended
            return;
        }
        ...
        $action = \Google\Web_Stories_Dependencies\sanitize_key($_GET['action']);
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        \Google\Web_Stories_Dependencies\check_admin_referer($action);
        $taxonomy_caps = (object) \Google\Web_Stories_Dependencies\get_taxonomy(self::TAXONOMY_SLUG)->cap;
        // Yes, cap is an object not an array.
        if (!\Google\Web_Stories_Dependencies\current_user_can($taxonomy_caps->manage_terms)) {
            return;
        }
        $referer = \Google\Web_Stories_Dependencies\wp_get_referer();
        $term_id = (int) $_GET['term_id'];
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $redirect = self::handle_validation_error_update($referer, $action, [$term_id]);
        if ($redirect !== $referer) {
            \Google\Web_Stories_Dependencies\wp_safe_redirect($redirect);
            exit;
        }
    }
    /**
    ...
    public static function handle_single_url_page_bulk_and_inline_actions($post_id)
    {
        ...
        $action = \Google\Web_Stories_Dependencies\sanitize_key($_REQUEST['action']);
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        $term_ids = isset($_POST['delete_tags']) ? \array_map('sanitize_key', $_POST['delete_tags']) : null;
        // phpcs:ignore WordPress.Security.NonceVerification.Missing
        $single_term_id = isset($_GET['term_id']) ? \Google\Web_Stories_Dependencies\sanitize_key($_GET['term_id']) : null;
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended
        ...
        \Google\Web_Stories_Dependencies\wp_safe_redirect(\add_query_arg($redirect_query_args, \Google\Web_Stories_Dependencies\get_edit_post_link($post_id, 'raw')));
        exit;
    }
    /**
    ...
    public static function handle_validation_error_update($redirect_to, $action, $term_ids)
    {
        ...
        global $pagenow;
        $has_pre_term_description_filter = \Google\Web_Stories_Dependencies\has_filter('pre_term_description', 'wp_filter_kses');
        if (\false !== $has_pre_term_description_filter) {
            \Google\Web_Stories_Dependencies\remove_filter('pre_term_description', 'wp_filter_kses', $has_pre_term_description_filter);
        }
        $updated_count = 0;
        ...
        if ($updated_count) {
            \Google\Web_Stories_Dependencies\delete_transient(\Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::NEW_VALIDATION_ERROR_URLS_COUNT_TRANSIENT);
        }
        return $redirect_to;
    }
    /**
    ...
    public static function handle_clear_empty_terms_request()
    {
        ...
        if (!\Google\Web_Stories_Dependencies\check_ajax_referer(self::VALIDATION_ERROR_CLEAR_EMPTY_ACTION, self::VALIDATION_ERROR_CLEAR_EMPTY_ACTION . '_nonce', \false)) {
            \Google\Web_Stories_Dependencies\wp_die(\esc_html__('The link you followed has expired.', 'amp'));
        }
        $taxonomy_caps = (object) \Google\Web_Stories_Dependencies\get_taxonomy(self::TAXONOMY_SLUG)->cap;
        // Yes, cap is an object not an array.
        if (!\Google\Web_Stories_Dependencies\current_user_can($taxonomy_caps->manage_terms)) {
            \Google\Web_Stories_Dependencies\wp_die(\esc_html__('You do not have authorization.', 'amp'));
        }
        $deleted_terms = self::delete_empty_terms();
        $referer = \Google\Web_Stories_Dependencies\wp_validate_redirect(\Google\Web_Stories_Dependencies\wp_get_raw_referer());
        if (!$referer) {
            return;
        }
        ...
        switch ($validation_error['code']) {
            ...
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INCORRECT_NUM_CHILD_TAGS:
                return \sprintf(\esc_html(
                    ...
                    \Google\Web_Stories_Dependencies\_n('Tag %1$s must have %2$s child tag', 'Tag %1$s must have %2$s child tags', (int) $validation_error['required_child_count'], 'amp')
                ), '<code>' . \esc_html($validation_error['node_name']) . '</code>', \esc_html(\Google\Web_Stories_Dependencies\number_format_i18n((int) $validation_error['required_child_count'])));
            case \Google\Web_Stories_Dependencies\AMP_Tag_And_Attribute_Sanitizer::INCORRECT_MIN_NUM_CHILD_TAGS:
                return \sprintf(\esc_html(
                    /* translators: %1$s is the node name, %2$s is required child count */
                    \Google\Web_Stories_Dependencies\_n('Tag %1$s must have a minimum of %2$s child tag', 'Tag %1$s must have a minimum of %2$s child tags', (int) $validation_error['required_min_child_count'], 'amp')
    ...
    public static function get_status_text_with_icon($sanitization, $include_reviewed = \false)
    {
        if ($sanitization['term_status'] & self::ACCEPTED_VALIDATION_ERROR_BIT_MASK) {
            $icon = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Icon::valid();
            $text = \__('Removed', 'amp');
        } else {
            $icon = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Icon::invalid();
            $text = \__('Kept', 'amp');
        }
        if ($include_reviewed) {
            $text .= ' (';
    ...
    {
        \Google\Web_Stories_Dependencies\delete_transient(self::TRANSIENT_KEY_ERROR_INDEX_COUNTS);
    }
}
