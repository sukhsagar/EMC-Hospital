...
class AMP_Validation_Manager
{
    ...
    {
        $service = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Services::get('dev_tools.user_access');
        return $service;
    }
    /**
     * Initialize.
    ...
    {
        if ($error && \Google\Web_Stories_Dependencies\amp_is_canonical()) {
    ...
    {
        if (\Google\Web_Stories_Dependencies\is_admin() || !self::get_dev_tools_user_access()->is_user_enabled() || !\Google\Web_Stories_Dependencies\amp_is_available()) {
        ...
        if ($is_amp_request) {
            ...
            $icon = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Icon::link();
            $href = $amp_url;
        }
        $icon_html = $icon->to_html(['id' => 'amp-admin-bar-item-status-icon', 'class' => 'ab-icon']);
        $parent = ['id' => 'amp', 'title' => \sprintf('%s %s', $icon_html, \esc_html__('AMP', 'amp')), 'href' => \esc_url($href)];
        // Construct admin bar item for validation.
        $validate_item = ['parent' => 'amp', 'id' => 'amp-validity', 'title' => \esc_html__('Validate URL', 'amp'), 'href' => \esc_url($validate_url)];
    ...
    public static function override_validation_error_statuses()
    {
        ...
        if (!isset($_REQUEST['_wpnonce']) || !\Google\Web_Stories_Dependencies\wp_verify_nonce(\Google\Web_Stories_Dependencies\sanitize_key($_REQUEST['_wpnonce']), self::MARKUP_STATUS_PREVIEW_ACTION)) {
            \Google\Web_Stories_Dependencies\wp_die(\esc_html__('Preview link expired. Please try again.', 'amp'), \esc_html__('Error', 'amp'), ['response' => 401]);
        }
        /*
        ...
        foreach ($_REQUEST[\Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::VALIDATION_ERRORS_INPUT_KEY] as $slug => $data) {
            ...
            $slug = \Google\Web_Stories_Dependencies\sanitize_key($slug);
    ...
    public static function init_validate_request()
    {
        ...
        if (\true === $should_validate_response) {
            ...
            if ($should_validate_response instanceof \WP_Error) {
                \Google\Web_Stories_Dependencies\wp_send_json(['code' => $should_validate_response->get_error_code(), 'message' => $should_validate_response->get_error_message()], 401);
            }
        }
    }
    /**
    ...
    {
        \Google\Web_Stories_Dependencies\add_action('wp', [__CLASS__, 'wrap_widget_callbacks']);
        \Google\Web_Stories_Dependencies\add_action('all', [__CLASS__, 'wrap_hook_callbacks']);
    ...
    {
        if (null === $user) {
            $user = \Google\Web_Stories_Dependencies\wp_get_current_user();
        }
        return \Google\Web_Stories_Dependencies\user_can($user, self::VALIDATE_CAPABILITY);
    }
    /**
     ...
     */
    protected static function has_dependency(\Google\Web_Stories_Dependencies\WP_Dependencies $dependencies, $current_handle, $dependency_handle)
    {
    ...
    {
        $dom = \Google\Web_Stories_Dependencies\AmpProject\Dom\Document::fromNode($node);
        ...
        if ($is_enqueued_link) {
            ...
            foreach (\wp_styles()->done as $style_handle) {
                ...
                foreach (self::$enqueued_style_sources as $enqueued_style_sources_handle => $enqueued_style_sources) {
                    if ($enqueued_style_sources_handle !== $style_handle && \wp_styles()->query($enqueued_style_sources_handle, 'done') && self::has_dependency(\wp_styles(), $enqueued_style_sources_handle, $style_handle)) {
                        ...
                        }, $enqueued_style_sources), $sources);
                    }
                }
            }
        }
            ...
            if ($node->hasAttribute('src')) {
                ...
                foreach (\wp_scripts()->done as $script_handle) {
                    ...
                    foreach (self::$enqueued_script_sources as $enqueued_script_sources_handle => $enqueued_script_sources) {
                        if ($enqueued_script_sources_handle !== $script_handle && \wp_scripts()->query($enqueued_script_sources_handle, 'done') && self::has_dependency(\wp_scripts(), $enqueued_script_sources_handle, $script_handle)) {
                            ...
                            }, $enqueued_script_sources), $sources);
                        }
                    }
                }
    ...
    protected static function handle_block_source_comment_replacement($matches)
    {
        ...
        $source = ['block_name' => $matches['name'], 'post_id' => \Google\Web_Stories_Dependencies\get_the_ID()];
        ...
        $block_type = \Google\Web_Stories_Dependencies\WP_Block_Type_Registry::get_instance()->get_registered($source['block_name']);
        if ($block_type && $block_type->is_dynamic()) {
            $callback_reflection = \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Services::get('dev_tools.callback_reflection');
    ...
    public static function wrap_hook_callbacks($hook)
    {
        ...
        foreach ($wp_filter[$hook]->callbacks as $priority => &$callbacks) {
            foreach ($callbacks as &$callback) {
                ...
                $wrapped_callback = self::wrapped_callback(\array_merge($callback, \compact('priority', 'source')));
                if (1 === $passed_by_ref) {
    ...
    public static function decorate_filter_source($value)
    {
        ...
        $post = \get_post();
        $source = ['hook' => \Google\Web_Stories_Dependencies\current_filter(), 'filter' => \true];
        if ($post) {
            $source['post_id'] = $post->ID;
            $source['post_type'] = $post->post_type;
        }
        if (isset(self::$current_hook_source_stack[\Google\Web_Stories_Dependencies\current_filter()])) {
            $sources = self::$current_hook_source_stack[\Google\Web_Stories_Dependencies\current_filter()];
            \array_pop($sources);
            // Remove self.
    ...
    {
        \Google\Web_Stories_Dependencies\_deprecated_function(__METHOD__, '2.0.2', '\\AmpProject\\AmpWP\\DevTools\\CallbackReflection::get_source');
        return \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Services::get('dev_tools.callback_reflection')->get_source($callback);
    }
    /**
    ...
    public static function can_output_buffer()
    {
        ...
        if (\Google\Web_Stories_Dependencies\did_action('shutdown')) {
            return \false;
        }
        ...
        if (\defined('DEBUG_BACKTRACE_IGNORE_ARGS')) {
            ...
            // phpcs:ignore PHPCompatibility.Constants.NewConstants.debug_backtrace_ignore_argsFound
        } else {
            $arg = \false;
        }
        $backtrace = \debug_backtrace($arg);
    ...
    {
        return \Google\Web_Stories_Dependencies\wp_hash(self::VALIDATE_QUERY_VAR . \Google\Web_Stories_Dependencies\wp_nonce_tick(), 'nonce');
    }
    /**
    ...
    {
        $data = ['results' => self::$validation_results, 'queried_object' => null, 'url' => \Google\Web_Stories_Dependencies\amp_get_current_url()];
        $queried_object = \Google\Web_Stories_Dependencies\get_queried_object();
        if ($queried_object) {
            $data['queried_object'] = [];
            $queried_object_id = \Google\Web_Stories_Dependencies\get_queried_object_id();
            if ($queried_object_id) {
                $data['queried_object']['id'] = $queried_object_id;
            }
            if ($queried_object instanceof \Google\Web_Stories_Dependencies\WP_Post) {
                $data['queried_object']['type'] = 'post';
            } elseif ($queried_object instanceof \Google\Web_Stories_Dependencies\WP_Term) {
                $data['queried_object']['type'] = 'term';
            } elseif ($queried_object instanceof \Google\Web_Stories_Dependencies\WP_User) {
                $data['queried_object']['type'] = 'user';
            } elseif ($queried_object instanceof \Google\Web_Stories_Dependencies\WP_Post_Type) {
     ...
     */
    public static function remove_illegal_source_stack_comments(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $dom)
    {
        /**
         * Script element.
         *
     ...
     */
    public static function finalize_validation(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $dom)
    {
        $total_count = 0;
        $kept_count = 0;
        $unreviewed_count = 0;
         ...
         */
        if (\Google\Web_Stories_Dependencies\amp_is_canonical()) {
            $dom->documentElement->removeAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP);
            $dom->documentElement->removeAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_EMOJI);
            $dom->documentElement->removeAttribute(\Google\Web_Stories_Dependencies\AmpProject\Attribute::AMP_EMOJI_ALT);
            /*
             ...
             */
            $script = $dom->createElement(\Google\Web_Stories_Dependencies\AmpProject\Tag::SCRIPT);
     ...
     */
    private static function update_admin_bar_item(\Google\Web_Stories_Dependencies\AmpProject\Dom\Document $dom, $total_count, $kept_count, $unreviewed_count)
    {
         ...
         */
        if ($kept_count > 0 && !\Google\Web_Stories_Dependencies\amp_is_canonical()) {
            $admin_bar_icon->setAttribute('class', 'ab-icon amp-icon ' . \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Icon::INVALID);
        } elseif ($unreviewed_count > 0 || $kept_count > 0) {
            $admin_bar_icon->setAttribute('class', 'ab-icon amp-icon ' . \Google\Web_Stories_Dependencies\AmpProject\AmpWP\Icon::WARNING);
        }
        ...
        if ($unreviewed_count > 0) {
            if ($unreviewed_count === $total_count) {
                ...
                $items[] = \Google\Web_Stories_Dependencies\_n('unreviewed', 'all unreviewed', $unreviewed_count, 'amp');
            } else {
                $items[] = \sprintf(
                    /* translators: %s the total count of unreviewed validation errors */
                    \Google\Web_Stories_Dependencies\_n('%s unreviewed', '%s unreviewed', $unreviewed_count, 'amp'),
        ...
        if ($kept_count > 0) {
            if ($kept_count === $total_count) {
                /* translators: text is describing validation issue(s) */
                $items[] = \Google\Web_Stories_Dependencies\_n('kept', 'all kept', $kept_count, 'amp');
            } else {
                $items[] = \sprintf(
                    /* translators: %s the total count of unreviewed validation errors */
                    \Google\Web_Stories_Dependencies\_n('%s kept', '%s kept', $kept_count, 'amp'),
        ...
        if (empty($items)) {
            /* translators: text is describing validation issue(s) */
            $items[] = \Google\Web_Stories_Dependencies\_n('reviewed', 'all reviewed', $total_count, 'amp');
        }
        $text = \sprintf(
            /* translators: %s is total count of validation errors */
            \Google\Web_Stories_Dependencies\_n('%s issue:', '%s issues:', $total_count, 'amp'),
    ...
    {
        $url = \Google\Web_Stories_Dependencies\amp_admin_get_preview_permalink();
        if (!$url) {
        ...
        if (\is_array($validity) && \count($validation_errors) > 0) {
            ...
            \Google\Web_Stories_Dependencies\delete_transient(self::PLUGIN_ACTIVATION_VALIDATION_ERRORS_TRANSIENT_KEY);
        }
    ...
    public static function validate_url($url)
    {
        ...
        for ($redirect_count = 0; $redirect_count < $allowed_redirects; $redirect_count++) {
            ...
            $response_code = \Google\Web_Stories_Dependencies\wp_remote_retrieve_response_code($r);
            $location_header = \Google\Web_Stories_Dependencies\wp_remote_retrieve_header($r, 'Location');
            ...
            $location_header = \Google\Web_Stories_Dependencies\wp_validate_redirect($location_header);
            if (!$location_header) {
                break;
            }
        ...
        $response = \trim(\Google\Web_Stories_Dependencies\wp_remote_retrieve_body($r));
        if (\Google\Web_Stories_Dependencies\wp_remote_retrieve_response_code($r) >= 400) {
            $data = \json_decode($response, \true);
            return new \WP_Error(\is_array($data) && isset($data['code']) ? $data['code'] : \Google\Web_Stories_Dependencies\wp_remote_retrieve_response_code($r), \is_array($data) && isset($data['message']) ? $data['message'] : \Google\Web_Stories_Dependencies\wp_remote_retrieve_response_message($r));
        }
        if (\Google\Web_Stories_Dependencies\wp_remote_retrieve_response_code($r) >= 300) {
    ...
    public static function get_validate_url_error_message($error_code, $error_message = '')
    {
        ...
        if (\version_compare(\get_bloginfo('version'), '5.2', '>=')) {
            $site_health_message .= \sprintf(
                ...
                \esc_url(\Google\Web_Stories_Dependencies\admin_url('site-health.php'))
            );
            $support_forum_message .= ' ' . \sprintf(
                /* translators: %s is the URL to Site Health Info. */
                \__('Please include your <a href="%s">Site Health Info</a>.', 'amp'),
                \esc_url(\Google\Web_Stories_Dependencies\admin_url('site-health.php?tab=debug'))
            );
        }
        $implode_non_empty_strings_with_spaces_and_sanitize = static function ($strings) {
            return \Google\Web_Stories_Dependencies\wp_kses(\implode(' ', \array_filter($strings)), ['a' => \array_fill_keys(['href', 'target'], \true), 'code' => []]);
        };
        switch ($error_code) {
            ...
            case 'fatal_error_during_validation':
                return $implode_non_empty_strings_with_spaces_and_sanitize([\esc_html__('A PHP fatal error occurred while validating the URL. This may indicate either a bug in theme/plugin code or it may be due to an issue in the AMP plugin itself.', 'amp'), \defined('Google\\Web_Stories_Dependencies\\WP_DEBUG_DISPLAY') && \Google\Web_Stories_Dependencies\WP_DEBUG_DISPLAY ? \esc_html__('The error details appear below.', 'amp') : $check_error_log . ' ' . \Google\Web_Stories_Dependencies\wp_kses_post(\sprintf(\__('Alternatively, you may enable %s to show the error details below.', 'amp'), '<code>WP_DEBUG_DISPLAY</code>')), $support_forum_message]);
    ...
    public static function print_plugin_notice()
    {
        ...
        if ('plugins.php' === $pagenow && (!empty($_GET['activate']) || !empty($_GET['activate-multi']))) {
            ...
            \Google\Web_Stories_Dependencies\delete_transient(self::PLUGIN_ACTIVATION_VALIDATION_ERRORS_TRANSIENT_KEY);
            $errors = \Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::summarize_validation_errors($validation_errors);
            $invalid_plugins = isset($errors[\Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::SOURCES_INVALID_OUTPUT]['plugin']) ? \array_unique($errors[\Google\Web_Stories_Dependencies\AMP_Validation_Error_Taxonomy::SOURCES_INVALID_OUTPUT]['plugin']) : null;
            if (isset($invalid_plugins)) {
                ...
                $more_details_link = \sprintf('<a href="%s">%s</a>', \esc_url(\add_query_arg('post_type', \Google\Web_Stories_Dependencies\AMP_Validated_URL_Post_Type::POST_TYPE_SLUG, \Google\Web_Stories_Dependencies\admin_url('edit.php'))), \__('More details', 'amp'));
                \printf(
                    ...
                    \esc_html(\sprintf(
                        ...
                        \Google\Web_Stories_Dependencies\_n('Warning: The following plugin may be incompatible with AMP: %s.', 'Warning: The following plugins may be incompatible with AMP: %s.', \count($invalid_plugins), 'amp'),
    ...
    public static function enqueue_block_validation()
    {
        ...
        if (\function_exists('Google\\Web_Stories_Dependencies\\wp_set_script_translations')) {
            ...
            $locale_data = \function_exists('Google\\Web_Stories_Dependencies\\wp_get_jed_locale_data') ? \Google\Web_Stories_Dependencies\wp_get_jed_locale_data('amp') : \Google\Web_Stories_Dependencies\gutenberg_get_jed_locale_data('amp');
            $translations = \wp_json_encode($locale_data);
            \Google\Web_Stories_Dependencies\wp_add_inline_script($slug, 'wp.i18n.setLocaleData( ' . $translations . ', "amp" );', 'after');
        }
    }
}
