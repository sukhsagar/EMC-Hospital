 ...
 */
final class TransformedIdentifierConfiguration extends \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Configuration\BaseTransformerConfiguration
{
    /**
    ...
    protected function validate($key, $value)
    {
        switch ($key) {
            case self::VERSION:
                if (!\is_int($value)) {
                    throw \Google\Web_Stories_Dependencies\AmpProject\Optimizer\Exception\InvalidConfigurationValue::forInvalidSubValueType(self::class, self::VERSION, 'integer', \gettype($value));
                }
                break;
        }
        return $value;
